...
TEST_CASE("BPINT 10 stops only on AX=0004 (CGA mode 4)")
{
    rex_session *s = rex_session_create();
    rex_regs_i8086 r{};
    REQUIRE(rex_session_load(s, "tests/fixtures/int10m4.com", nullptr) == REX_OK);
    ...
    REQUIRE(r.ip == 0x0103);
    REQUIRE(r.ax == 0x0004);
    /* Second INT 10 is AH=0Fh — filter skips it so we reach INT 20. */
    REQUIRE(rex_session_run(s, 10000) == REX_OK);
    REQUIRE(rex_session_halted(s));
    rex_session_destroy(s);
}

TEST_CASE("BPINT 10 once stops only on the first mode-4 INT 10")
{
    rex_session *s = rex_session_create();
    rex_regs_i8086 r{};
    rex_int_bp ib[4]{};
